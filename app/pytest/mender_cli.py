...
class MenderCli:
    ...
    def login(self, timeout=60):
        ...
        try:
            ...
            print(f"🔴​​​ Failed to login, mender-cli failed with exit code {e.returncode}, stdout {e.stdout} and stderr {e.stderr}")
            raise
    ...
    def shell_exec(self, device, cmd, timeout=60):
        ...
        try:
            ...
            print(f"🔴​​​ Failed to execute command, mender-cli failed with exit code {e.returncode}, stdout {e.stdout} and stderr {e.stderr}")
            raise
        return result
